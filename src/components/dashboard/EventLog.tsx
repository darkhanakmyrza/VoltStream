...
import type { EventLogProps, EventSeverity } from "@/types/battery";
...
export function EventLog({ events }: EventLogProps) {
  return (
    <aside className="panel flex min-h-[420px] flex-col p-6 sm:p-7">
      ...
      <div className="mb-4 inline-flex w-fit items-center gap-2 rounded-full border border-zinc-800 bg-zinc-950/65 px-3.5 py-1.5 text-xs uppercase tracking-[0.18em] text-zinc-400">
      ...
      <div className="volt-scrollbar -mr-2 flex-1 space-y-3 overflow-auto pr-2">
        {events.map((event) => (
          <article key={event.id} className="rounded-2xl border border-zinc-800/80 bg-zinc-950/60 p-4">
